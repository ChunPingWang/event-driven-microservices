...
import com.example.paymentservice.domain.event.handler.PaymentProcessedEventHandler;
import com.example.paymentservice.domain.event.handler.PaymentFailedEventHandler;
...
public class DomainEventDispatcher {
    ...
    private final PaymentProcessedEventHandler paymentProcessedEventHandler;
    ...
    public void dispatchEvents(AggregateRoot aggregateRoot) {
        ...
        log.info("Dispatching {} domain events from payment aggregate", events.size());
    ...
    private void dispatchEvent(DomainEvent event) {
        ...
        if (event instanceof PaymentProcessedEvent paymentProcessedEvent) {
            log.info("Dispatching PaymentProcessedEvent for payment: {}", 
                paymentProcessedEvent.getPaymentId().getValue());
            paymentProcessedEventHandler.handle(paymentProcessedEvent);
        } else if (event instanceof PaymentFailedEvent paymentFailedEvent) {
            log.info("Dispatching PaymentFailedEvent for payment: {}", 
                paymentFailedEvent.getPaymentId().getValue());
            paymentFailedEventHandler.handle(paymentFailedEvent);
        } else {
