...
import com.example.orderservice.domain.event.DomainEventDispatcher;
...
/**
 * 領域事件發布器適配器 - 實現出站端口接口，將領域事件調度到相應的處理器
 */
@Component
...
public class DomainEventPublisherAdapter implements DomainEventPublisher {
    ...
    private final com.example.orderservice.domain.event.DomainEventPublisher domainEventPublisher;
    ...
    public void publishEvents(List<DomainEvent> events) {
        ...
        log.info("Publishing {} domain events through adapter", events.size());
        ...
        try {
            ...
            log.error("Failed to publish domain events through adapter", e);
